...
5. Success: a dialog shows the file path and a **白酒分析** / **Baijiu Analysis** button (default). Status line also shows the path. The live CSD editor from Start stays; the workstation does **not** open a second chromatogram tab for the same file. If no live editor is open (edge case), the saved file is opened once. The right sash stays **one 白酒操作** (plant-home singleton); Stop/save/handoff must not clone `part.workbench.plantHome` or add the community shared workbench part. Optional auto-open of **白酒工作台** (`handoff.autoOpenBaijiu`) is unchanged and is not a duplicate CSD tab. Main **开始分析 → 启动** is unchanged (no extra Baijiu button on the reverse-control panel). The current sequence row is marked **已完成** and linked to that file; the pointer advances.
