...
5. Success: a dialog shows the file path and a **白酒分析** / **Baijiu Analysis** button (default). Status line also shows the path. The live CSD editor from Start stays and is rebound to the saved file (tab label becomes the saved basename, e.g. `GC-FID_yyyyMMdd_HHmmss [CSD]`). The workstation does **not** open a second chromatogram tab for the same file — neither the native save path nor **白酒分析** / auto-open handoff (`BaijiuHandoffBridge.open` → `OpenBaijiuChromatogramHandler.openFile`). If no live editor is open (edge case), the saved file is opened once. If the live editor cannot be rebound, it is closed and the file is opened once. The right sash stays **one 白酒操作** (plant-home singleton); Stop/save/handoff must not clone `part.workbench.plantHome` or add the community shared workbench part. Optional auto-open of **白酒工作台** (`handoff.autoOpenBaijiu`) is unchanged and is not a duplicate CSD tab. Main **开始分析 → 启动** is unchanged (no extra Baijiu button on the reverse-control panel). The current sequence row is marked **已完成** and linked to that file; the pointer advances.
6. Click **白酒分析** (or press Enter): the workstation switches to the **白酒工作台** perspective (E4 workbench, same as **窗口 → 视图 / 插件 → 白酒工作台**) and **reuses** the live chromatogram editor in 谱图/采集 (does not call ChemClipse `openEditor` on the file when that editor is already open). Continue as usual: 推荐积分 → 白酒分析 → GB 2757 / 报告.
