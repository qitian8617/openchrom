...
- Starts on **厂工作台**. Horizontal sash (resizable): **left** column is itself a vertical sash — **upper** PartStack of workflow pages (推荐积分 / 白酒分析 / 三步向导 / 进样序列 / 批处理结果 / 简单批量 / 平行样 / 预览报告) and **lower** **谱图 / 采集** (ChemClipse editor Area + empty-state, not closable; open CSD and live acquisition land here, not in the upper tab row); **right** fixed **白酒操作** sidebar (NoDetach). Reverse-control is an **independent window** shown/hidden by toolbar **反控** (check item, **default hidden**; close hides, reopen shows the same singleton; hide remembered via `BaijiuGcConsoleHidden`). Community install still opens the floating dialog.
- Plant-home hosts are branding Parts (`BaijiuGcHomePart` / `BaijiuSequenceHomePart` / `BaijiuAnalysisHomePart` / `BaijiuWorkbenchHomePart`) that OSGi-load the real panels. Chromatogram host is `placeholder.plantChromatogram` in the lower-left `partstack.plantChromatogram` → ChemClipse Area (not the dead `placeholder.plantEditor`, not a right-side tab, not an upper workflow tab). Cold start must show the plant toolbar (打开谱图 / 反控), the upper workflow pages, and the lower 谱图/采集 placeholder, **or a readable error Label** — never blank gray.
- Top menu **文件 / 白酒 / 视图 / 帮助**. 处理器 / 插件 are hidden. Toolbar: **打开谱图**、**反控**、开始分析、推荐积分、定量/白酒分析、报告. **开始分析** / Main「启动」 keeps the live chart on the **lower-left** 谱图/采集 surface after a successful start. 白酒分析 actions that need 当前谱图 use the selected tab in that lower stack; the analysis page header shows that file name.
...
- [ ] **Dedicated product:** Run As / Export `openchrom.compilation.baijiu.product`; title 白酒 FID 工作站; starts on **厂工作台** (toolbar 打开谱图 + 反控 unchecked; **upper-left** workflow pages; **lower-left** 谱图·采集 placeholder; **right** 白酒操作 only; FID console independent window **default hidden** until 反控, or a readable error Label — never blank gray); opening a CSD and **开始分析** both land in the **lower-left** 谱图/采集 stack (tabs stay there, not beside 白酒分析); restart keeps the vertical sash; top bar without 处理器/插件; `baijiu.ui` still JavaSE-21
