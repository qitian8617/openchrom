...
1. **默认主屏** = **进样序列 / 白酒分析 / 谱图·采集** 工作流选项卡；反控（气/火/信号/就绪）在左侧可开关 sash，工具条 **反控** 显示/隐藏
...
│  专用壳（本产品拥有）                                      │
│  · 窗口标题「白酒 FID 工作站」                              │
│  · 启动视角：厂工作台（主区选项卡：进样序列 / 白酒分析 / 谱图·采集；反控可开关）│
...
|--------|------|-------------------|
| **壳** | 厂工作台视角、分析页视角、菜单/工具栏裁剪、布局 epoch=10、顶栏「白酒」 | **改** branding 插件 + `.product` |
...
| 当前针 + 序列表 | **厂工作台主区第一页签「进样序列」** | 壳 `BaijiuSequenceHomePart` OSGi 加载 `BaijiuSequenceComposite` |
| 打开谱图 | **打开谱图** 工具条 / **白酒 → 打开谱图** / 文件 → 打开 CSD | ChemClipse CSD；`.ocb`；打开后切到 **谱图 / 采集** 页签 |
| 开始分析 | **开始分析** 工具条 / 白酒菜单 / 反控 Main「启动」 | `AcquisitionStartGate`（与 Main 同一 FID 门）；成功后切到 **谱图 / 采集** 大图 |
...
1. **启动就在厂工作台**（主区选项卡：进样序列 / 白酒分析 / 谱图·采集；反控 sash 默认可关）。不要 Welcome / MALDI / HPLC-DAD / Data Analysis。打开谱图或开始分析后切到谱图页签（ChemClipse 编辑器 Area）。
...
| 情况 | 行为 |
|------|------|
| 第一次升到 Phase 3 chrome epoch（当前 = 10） | 自动清一次旧 `workbench.xmi`，然后记住新布局 |
...
- 本仓 Cloud Agent 环境通常 **不能** 弹出 Windows SWT 工作站做点击验收；厂工程师按 README 在本机 PDE 验证。冷启动（epoch=10 会再清一次 `workbench.xmi`）后必须能看到厂工具条（打开谱图 / 反控）和主区工作流页签（进样序列为默认），反控 sash 默认可关，**或页内可读错误 Label**，不能再是空灰。
