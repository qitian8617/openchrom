...
|--------|------|-------------------|
| **壳** | 厂工作台视角、分析页视角、菜单/工具栏裁剪、布局 epoch=3、顶栏「白酒」 | **改** branding 插件 + `.product` |
...
|------|------------------------|----------|
| 气/火/信号/就绪 | **启动即厂工作台左侧反控**（Main 气/火/信号/FID 就绪） | `temperature.ui` `TemperatureControlPart`；软件不控气路 |
| 当前针 + 序列表 | **厂工作台右侧进样序列 Part**（同一屏，不是侧页签才看得到） | `baijiu.ui` `BaijiuSequencePart` 宿主现有 `BaijiuSequenceComposite` |
