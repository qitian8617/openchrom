...
|--------|------|-------------------|
| **壳** | 厂工作台视角、分析页视角、菜单/工具栏裁剪、布局 epoch=9、顶栏「白酒」 | **改** branding 插件 + `.product` |
...
1. **启动就在厂工作台**（状态 + 序列；无 plantEditor 谱图占位）。不要 Welcome / MALDI / HPLC-DAD / Data Analysis。谱图在「打开谱图」后进分析页/编辑器。
...
| 情况 | 行为 |
|------|------|
| 第一次升到 Phase 3 chrome epoch（当前 = 9） | 自动清一次旧 `workbench.xmi`，然后记住新布局 |
...
- 本仓 Cloud Agent 环境通常 **不能** 弹出 Windows SWT 工作站做点击验收；厂工程师按 README 在本机 PDE 验证。冷启动（epoch=9 会再清一次 `workbench.xmi`）后主区必须能看到反控和/或序列表，**或页内可读错误 Label**，不能再是空灰。
